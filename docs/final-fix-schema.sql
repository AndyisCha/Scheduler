...
-- 2. generated_schedules 테이블 RLS 정책 (이 테이블이 있다면)
DO $$
BEGIN
...
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'slots') THEN
        ...
        -- owner_id 또는 user_id 컬럼 기반 정책 생성 (실제 컬럼명 확인 필요)
        -- 먼저 컬럼 존재 여부 확인
        ...
        ELSE
            RAISE NOTICE 'slots 테이블에 owner_id 또는 user_id 컬럼이 없습니다. 컬럼 구조를 확인해주세요.';
        END IF;
          
        RAISE NOTICE 'slots RLS 정책이 적용되었습니다.';
    ELSE
        RAISE NOTICE 'slots 테이블이 존재하지 않습니다.';
    END IF;
END $$;

-- 4. teacher_pools 테이블 RLS 정책 (있다면)
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'teacher_pools') THEN
        ...
        -- owner_user_id 값 업데이트 (slots 테이블을 통해)
        -- slots 테이블의 실제 컬럼명에 따라 업데이트
        IF EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'slots' AND column_name = 'owner_id') THEN
